...
```
The second model we consider uses all possible characteristics including property characteristics, district characteristics and spatial characteristics. 

```{r}
ols_1 <- lm(Price_per_sqm ~ 
            District + Days_from_initial_announcement_binned + Rent_imp + Ownership + Floor_binned + Construction_type +
            Finishing_condition + No_of_floors_binned + Property_market + Area + No_of_rooms + Building_age_binned  + 
            Heating + Population_density + Vistula_side + Life_quality + Family_friendliness + Single_people_friendliness +
            Senior_people_friendliness + Distance_to_PKIN_in_km + Distance_to_subway_in_km + Distance_to_airport_in_km,
          data=df_final)
```

With the use of stepwise regression we obtain the final model formula which considers all the passed variables. It is clear that District is a significant factor - districts like Białołęka, Rembertów, Wawer or Wesoła indicate lower property prices than those districts closer to the center like Śródmieście, Ochota, Mokotów or considered as family friendly like Ursynów, Wilanów or Bielany. The model 
