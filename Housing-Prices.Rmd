...
packages_vector = c("knitr", "readxl", "kableExtra", "ggplot2", "rpart", "rattle", "mice", "dplyr","forcats", "spdep", "rgdal", "maptools", "sp", "RColorBrewer", "e1071", "spatstat", "GISTools", "caret")
...
<p class = "noindent"> When analyzing variable indicating construction completion year, we noticed three outliers which seemed to be caused by mistyping. We also decided to create and use further in the modelling variable indicating age of the building instead of the year it was built. </p>
...
This section addresses missing data treatment. Below variables with missing values are presented along with the missings ratio. 
...
missings %>% dplyr::filter(Missings_ratio > 0.001) %>% dplyr::select(Variable, Missings_ratio)
```

<p class = "noindent"> For qualitative variables like form of ownership, floor, type of construction, finishing condition, heating, number of floors in the building, building age and floor, we decide to make missing values an explicit factor level. </p>
...
<p class = "noindent"> As to qualitative variables, we decided to fill the value of Rent with Predictive according to Mean Matching algorithm. Value of rent for the owned property is a concept typical for Poland and depends on many external factors like type of the entity managing the building, living area, number of people living in the property, type of heating (e.g. if urban then it is contained in the rent, if gas then it is paid separately to the gas plant). We decided to use as a proxy of these factors the living area, number of rooms and also type of heating. </p>
...
First model we run is linear regression with the use of only spatial characteristics as independent variables. We consider stepwise regression in order to obtain the best model formula according to AIC criterion.
...
The second model we consider uses chosen property characteristics without any missing values and also spatial characteristics. 
...
With the use of stepwise regression we obtain the final model formula which considers selected variables (variables Vistula_side and Distance_to_subway_in_km were not chosen by the algorithm). It is clear that District is a significant factor - districts like Białołęka, Rembertów, Wawer or Wesoła indicate lower property prices than those districts closer to the center like Śródmieście, Ochota, Mokotów or considered as family friendly like Ursynów, Wilanów or Bielany. The model also indicate that relationship however in this case it should be interpreted with respect to the base factor level being Bemowo district. As to property market, properties from the secondary market tend to exhibit higher prices per squared metre than those from the primary market. Relationship of price for squared metre and the living area of the property is positive but rather small meaning with the increase of the living area also price for squared metre is higher. As to number of rooms, the model indicated that price for squared metre is always lower for apartment with more than one room. The influence of spatial variables is the same as in case of the first model. 
...
Another type of models we consider are spatial models. Due to the fact that the data we analyze are point data, we needed to somehow aggregate it. Instead of analyzing particular districts, we decided to divide Warsaw map into grid with cells of 1 km x 1 km and then aggregate the data within grid cells. 
...
Having read and transformed data on grids, we are now able to limit grid data for whole Poland to only Warsaw. The operation resulted in obtaining 601 grids which can be seen on the plot below compared to the plain districts visualization. 
...
As a next step we needed to transform data on property characteristics to spatial object in order to be able to merge it with the grid data. We decided to use only chosen characteristics which could still be potentially sensible predictors after aggregation obtained with the use of mean value. These variables are rent, building age, distance to PKiN, distance to nearest subway station and also distance to airport. Dependent variable is price of the property per squared metre. Plot below presents the average property price per squared metre per grid.
...
colnames(pop.df.warsaw)[5:10] <- c("Price_per_sqm", "Rent", "Building_age", "Distance_to_PKIN_in_km", "Distance_to_subway_in_km", "Distance_to_airport_in_km")
...
Having prepared final dataset, we move to the modelling part. We decided to use contiguity spatial weights matrix and run Manski model with full specification meaning with the spatial lag of the dependent variable, spatial lag of the explanatory variables and spatial lag of error. Below can be seen the full model equation. We estimate the model with the use of spdep::sacsarlm() function.
...
# model formula
formula <- formula(Price_per_sqm ~ Rent + Building_age + Distance_to_PKIN_in_km + Distance_to_subway_in_km + Distance_to_airport_in_km) 

# Manski model
gns <- sacsarlm(formula, data=pop.df.warsaw, listw=cont.listw, type="sacmixed")
```

Since Manski model contains a spatial lag of the dependent variable, we should instead of beta coefficients interpret indirect and direct effects. Direct effect indicate the influence of explanatory variable $x$ in location $i$ on the value of dependent variable in location $i$. Indirect effect indicate the influence of explanatory variable $x$ in location $i$ on the value of dependent variable in location $j$. Total effect is a sum of both direct and indirect effect and thus a total effect of explanatory variable indicating the influence of explanatory variable $x$ in location $i$ on the value of dependent variable in locations $i$ and $j$. 

In case of Rent variable the total effect is additive but rather weak. Indirect effect is higher than direct effect which indicates that higher rent in a particular location influences more (positively) the price per squared metre in all other locations. As to building age, it influences positively 4 times more the price per squared metre in the other locations then price per squared metre in the same location. 

Moving to the spatial characteristics there are following conclusions:  
- the longer the distance to PKIN, the higher price per squared metre in the same location and the lower price per squared metre in all other locations;  
- the longer the distance to the nearest subway the lower the price in the same location and the higher price in all other locations;  
- the longer the distance to airport the higher the price in the same location and the lower the price in all other locations.  
...
The last type of model we run is Extreme Gradient Boosting (XGBoost). XGBoost is an ensemble algorithm based on boosting technique. Firstly, ensemble is a concept in which the idea is to train multiple models using the same learning algorithm. In case of boosting the logic is to build subsequent predictors which learn from the mistakes of the previous predictors. Therefore, the observations in the training dataset have an unequal probability of appearing in subsequent models (in case of XGBoost these are regression trees). Observations which in previous models were classified wrong will have higher probability to appear in the following models than observations classified correctly. Because new models are learning from mistakes committed by previous models, it takes less iterations to reach very good predictions. 

The technical problem with XGBoost is that this algorithm is likely to overfit. Thus several parameters (called hyperparameters) are set to control the fitting process. Below it can be seen the summary of important model hyperparameters needed to specify when building XGBoost. 

```{r echo = TRUE, message=FALSE}
modelLookup("xgbTree")
```

In order to build XGBoost model, we pass all variables since the algorithm will decide itself which ones are the most important and based on that they will be chosen more frequently as the splitting features in the decision trees. The model formula includes all original variables existing in the dataset. 
...
library(xgboost)
xgb_train_1 <- train(
  x = as.matrix(train.df_final %>%
                  dplyr::select(-Price_per_sqm)),
  y = as.factor(train.df_final$Price_per_sqm),
